...
isolated function triggerModuleRelease(Module m) returns int|error {
    ...
    // boolean|error versionAvailable = trap isVersionAlreadyAvailable(m);
    // if versionAvailable is error {
    //     printError(versionAvailable);
    //     return 0;
    // }
    // if versionAvailable {
...
isolated function isVersionAlreadyAvailable(Module m) returns boolean {
    ...
    printInfo(string `Checking the released version of the module: ${m.name}: ${m.module_version}`);
    string version = string `v${m.module_version}`;
    github:Release|error release = trap github->/repos/[GITHUB_ORG]/[m.name]/releases/tags/[version];
    if release is error {
        return false;
    }
    printInfo(string `Found release: ${release.tag_name} for module: ${m.name}`);
    return true;
}
