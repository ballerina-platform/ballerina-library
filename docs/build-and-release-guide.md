...
8. Then the release manager can trigger the [Library Release Workflow](https://github.com/ballerina-platform/ballerina-library/actions/workflows/release_pipeline.yml) to trigger the release.
    ...
    - `release_libs`: To release the library (`module-ballerina-*`) modules, whether or not they are packed with the distribution. These modules are listed under the `library_modules` field in the `stdlib_modules.json` file. The default value is `true`.
