...
5. Reuse a healthy code-server on a user-specified port or port 8080 when the user provides its access credentials. Otherwise generate a per-run `CODE_SERVER_TOKEN` with `python3 -c 'import secrets; print(secrets.token_urlsafe(32))'`, keep it only in the current agent's context, and start `PASSWORD="$CODE_SERVER_TOKEN" code-server --auth password --bind-addr 127.0.0.1:PORT SAMPLE_PARENT`. Redirect output to `run-log/code-server.log` and record its PID, but never copy the token into artifacts, logs, screenshots, or the final report. Authenticate the browser session with `CODE_SERVER_TOKEN`. Stop only a server started by this run, then discard the token.
