...
- **Script is much slower than the ~20-40s/query above** — check `check_triggered()` still passes `--permission-mode bypassPermissions`; without it, denied `Read`/`Bash` calls send the model into retry loops that can stretch a single query to several minutes.

## Iterating on the description

1. Run `train_queries.json`, look at which queries failed.
2. Revise `SKILL.md`'s `description` field to address the *general* gap the failures point to — not specific keywords from the failed queries (that's overfitting).
